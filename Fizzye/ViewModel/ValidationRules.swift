...
struct StringValidationRule: ValidationRule {
    func validate(_ input: String) -> String? {
        ...
        return isValidFormat ? nil : "The code must be in the format 'L1234'."
    }
}
...
struct IncrementalValidationRule: ValidationRule {
    func validate(_ input: String) -> String? {
        if input.count > 5 {
            return " The code cannot exceed 5 characters."
        }
        if let firstChar = input.first, !firstChar.isLetter {
            return "The code must start with a letter."
        }
        if input.count > 1{
            for char in input.dropFirst() {
                if !char.isNumber {
                    return "The remaining characters must consist of numbers only."
                }
            }
        }
       return nil
